...
export function seedQueries(product = {}) {
  ...
  return [...new Set(seeds.map(value => String(value).replace(/\s+/g, " ").trim()).filter(Boolean))].slice(0, 24);
}
...
export function buildDataset({ previous = {}, products, captured, asof }) {
  ...
  return {
    _comment: "네이버 공개 검색 화면 자동완성 표시어와 구글형 질문·비교 롱테일 보강 후보의 월간 스냅샷. visible_ui만 실제 화면 관측이며, expanded_seed/pattern_expand는 후보 확장값이다.",
