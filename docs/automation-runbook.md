...
| pages.yml | Deploy to GitHub Pages | push·dispatch | — | (배포) | ✖ | `pages` |
...
```
오전:  signals(06:00) → news-clip(07:30) → daily-brief(08:00)
오후:  news-clip(13:00) → daily-brief(14:00)
```
...
1. **공유 concurrency 레인** `cm-cmo-data-writers` — 커밋/푸시하는 6개 워크플로가 같은 그룹을 사용해 GitHub가 **직렬화**(동시에 하나만 실행). `cancel-in-progress: false`로 어떤 실행도 버리지 않는다.
...
## workflow_run을 쓰지 않은 이유

- 오전 브리프는 **두 수집(signals + news)** 이 모두 끝나야 이상적이나, `workflow_run` 하나로는 두 upstream의 **fan-in**이 되지 않고(단일 워크플로 완료만 트리거), 중복 실행·경합이 생기기 쉽다.
- P0-1이 브리프에서 신선도를 실시간 계산하므로 하드 순서 보장이 **정확성**엔 불필요(늦으면 stale로 표시). → 가장 단순하고 실패 확률 낮은 **cron 간격 + 공유 레인 + 안전 push** 조합을 선택.

## 유지보수 주의

- **`concurrency.group` 이름을 바꾸면** 직렬화가 깨진다. 6개 커밋 워크플로는 반드시 동일 그룹(`cm-cmo-data-writers`)을 유지할 것.
