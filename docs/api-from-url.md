...
> 보안(인증·요청 허용 정책): 워커는 **허용 출처(`ALLOW_ORIGINS`) 화이트리스트**의 Origin/Referer에서 온 요청만 처리합니다(그 외 **403**). 키는 **워커 시크릿만** 사용하고 **브라우저에서 키를 받지 않습니다**(헤더 override 폐지 → 브라우저 키 노출 0). 또 **라우트·메서드 화이트리스트**(정의된 엔드포인트만, 그 외 404)와 (KV 있을 때) **IP·일 요청 상한**(초과 429)이 적용됩니다. 커스텀 도메인을 쓰면 `ALLOW_ORIGINS` 에 추가하세요.
