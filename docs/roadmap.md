...
- ✅ **일일 파이프라인 재정비·보안**(2026-08-07) — 순서 06:30→07:20→07:40→08:00 등, 커밋 워크플로 공유 레인 + 읽기 전용 알림 레인 분리 / 브라우저 API 키 입력·저장 제거 + 워커 인증·요청허용 정책(Origin 화이트리스트·시크릿 전용·라우트/메서드 제한·레이트리밋).
