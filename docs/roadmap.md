...
### C. 운영 자동화 (구현됨/진행)
- ✅ **데일리 카카오 비서** — 매일 08:00 KST 브리핑(시즌+뉴스+SERP 할일). 설정: [daily-brief.md](daily-brief.md).
- 후보: 경쟁사 리뉴얼 키워드 뉴스 감지 → 카카오 알림(사내 news_watch 패턴 이식).

## 우선순위 제안
1. `data/products.json` 단일 마스터화(기술부채 해소).
2. SERP 전/후 **시각 diff 자동 변화 감지**(현업 체감 큼).
3. 시즌 캘린더 **데이터랩 실측 검증** 연동.
