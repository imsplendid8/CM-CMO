---
name: cm-faq-editor
description: SearchAd 검색량과 선택적 Search Console 검색어에서 실제 고객 질문형 FAQ 기회를 발굴한다. FAQ SEO, 질문 발굴, 콘텐츠 공백, FAQ JSON-LD 초안 요청에 사용한다.
---
...
4. 자동화는 질문 후보만 제안하고 답변은 생성하지 않는다.
5. 답변은 첫 문장에서 직접 답하고 다음 문장에서 조건·예외·확인 경로를 설명한다.
6. 화면 FAQ와 JSON-LD 문구는 동일하게 유지한다.
7. 답변을 작성할 때는 최신 상품자료·약관을 확인하고 상품 담당·준법·광고심의 전에는 게시 가능 상태로 바꾸지 않는다.
