...
Use this reference when the user wants a private admin, material factory, review queue, approval table, rejected-copy reasons, or quality improvement from operator feedback.

## Purpose

The review lab is a private operator workspace, not a public publishing system. It stores generated SA copy, Power Content concepts, thumbnail prompts, bitmap outputs, review decisions, and rejection reasons so future generations stop repeating weak patterns.
...
- `product`
- `month`
- `channel`: `sa_copy`, `powercontent`, `thumbnail`, `banner`, `keyword`
- `source_inputs`: SERP snapshot id, news summary id, Power Content concept id, keyword set id, product text id
- `generated_material`: copy text, article section, image brief, prompt, or asset path
- `naver_constraints_checked`: true/false plus notes
...
- `naver_limit_violation`: title, description, additional description, promotion, or sitelink violates a Naver limit.
- `serp_gap_not_reflected`: structured SERP finding did not influence the material.
- `powercontent_mismatch`: SA copy does not connect to the current Power Content concept.
...
1. Approved patterns to preserve.
2. Rejected patterns to avoid.
3. Repeated reason codes.
4. Product-specific caution points.
5. Open review questions.

Then generate a materially different set. Do not simply replace one adjective with another.
...
- the gap is structural, such as missing SERP text fields or no duplicate control.
