...
- SA 소재는 심의안에 없는 네이버 확장소재 추가설명 4개도 별도 제안한다. 내보내기는 심의안+추가소재 초안과 내부 검토용 CSV로만 제공하며 광고주센터 공식 템플릿 열 매핑은 제공하지 않는다.
- “바로 등록”, “업로드 가능”, “심의 통과”를 자동으로 표시하지 않는다.
- 키워드 도구의 대량등록 파일은 실제 광고주센터 샘플로 인코딩과 열 구성을 추가 검증해야 한다.
- 파워콘텐츠 도구는 선택 상품의 SearchAd 실측 연관어와 대표·담보·의사결정·질문·시즌 키워드를 중복·범위 필터 후 전체 키워드셋 Excel로 제공한다.
